...
- ランチャと結果画面は topbar / progress track / 本体 / status bar の4帯。**コード編集画面はこの furniture を使わない。** 専用の workspace bar（［← 戻る］／画面名とセッション名と状態バッジ／［検証］［実行］［集中モード］）を自分で持ち、topbar・テーマ切替・health badge は載せない。編集に関係しない chrome をエディタの上へ積まない。
...
- ［集中モード］は **形式切替・版ボタン・AI 相談列・status bar を実際に外し**、モジュールとエディタだけにする。OS のフルスクリーンにはしない。両 layout は**同じ editor object を持ち回す**ので、往復しても本文・選択 module・カーソル・スクロールが失われない。
